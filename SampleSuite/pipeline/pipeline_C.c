...
void *worker(void *arg){
    ...
    enter_block(2, worker_id, "for(int i = 0; i < data_size; i++)");
    for(int i = 0; i < data_size; i++){
        enter_block(3, worker_id, "for(int i = 0; i < data_size; i++)");
            basic_block_id = enter_block(4, worker_id, "data[i] = data[i] + 1 + previous_worker_state");

                data[i] = data[i] + 1 + previous_worker_state;

            data_flow_trace(shadow_previous_worker_state, basic_block_id, worker_id);
            data_flow_trace(shadow_data[i], basic_block_id, worker_id);
            shadow_data[i] = basic_block_id;
            exit_block(worker_id);
            basic_block_id = enter_block(5, worker_id, "previous_worker_state = worker_state");
            ...
            exit_block(worker_id);
            basic_block_id = enter_block(6, worker_id, "worker_state = worker_state + 1;");
