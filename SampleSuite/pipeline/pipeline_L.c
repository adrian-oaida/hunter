...
void *worker(void *arg){
    ...
    for(int i = 0 ; i < data_size; i++){
        ...
        switch(worker_id % 3){
            case 0:
                basic_block_id = enter_block(4, worker_id, "stage1_state += data[i]");
                stage1_state += data[i];
            ...
            case 1:
                ...
                basic_block_id = enter_block(7, worker_id, "while(tmp != 0)");
                while(tmp != 0){
                    basic_block_id = enter_block(8, worker_id, "tmp = tmp / 3");
                    tmp = tmp / 3;
                ...
                break;
            case 2:
                basic_block_id = enter_block(10, worker_id, "stage2_state+= data[i]");
                stage3_state+= data[i];
                ...
                exit_block(worker_id);
                basic_block_id = enter_block(11, worker_id, "if(stage3_state % 3 == 0)");
                if(stage3_state % 3 == 0){
                    basic_block_id = enter_block(12, worker_id, "data[i] = stage3_state + data[i]");
                    ...
                    basic_block_id = enter_block(13, worker_id, "data[i]++");
                    data[i]++;
                    data_flow_trace(shadow_data[i], basic_block_id, worker_id);
                    shadow_data[i] = basic_block_id;
                    exit_block(worker_id);
